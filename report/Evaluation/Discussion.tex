...
Then comparing our solution to others, such as Cucumber and Fitnesse, there are some key differences.
Where both Cucumber and Fitnesse work by defining behaviour in plain text, however still using their representative syntaxes.
Where both alternatives use text-based descriptions of behaviour our system does instead use the graphical interface of the mobile application to allow for true "drag 'n drop" interaction.
...
These truth tables as then compared with the evaluated values from the system ans is showed in the Wiki User Interface that Fitnesse uses.
...
With our solution the developer simply needs to generate some new tests, if they have changes, and import those tests into the software project.
...
Where Fitnesse is a rather old system, from 2005 \cite{FitnesseDownload}, and it has not seen mass adoption by developers.
Where Cucumber is a rather new alternative, from 2018 \cite{CucumberWiki}, has seen some adoption, however, mostly to large companies where they have in-house development teams developing their products.
We see this as an effect of their Product Owners being in close relations to their developers allowing for the Product Owners to ether learn the syntax or create it in collaboration with the developers.
